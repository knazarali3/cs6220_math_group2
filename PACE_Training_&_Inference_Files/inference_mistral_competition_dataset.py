...
def format_competition_math(test_dataset, tokenizer):
    ...
    return test_dataset.map(formatting_prompts_func)  # Process examples individually

def mistral_competition_inference_with_resume():
    max_seq_length = 2048  # supports RoPE Scaling internally
    dtype = None  # None for auto detection
    load_in_4bit = True  # Use 4bit quantization to reduce memory usage.

    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name="SaalihaA/CS6220_Mistral_Finetune",  # YOUR MODEL YOU USED FOR TRAINING
    ...
    test_dataset = format_competition_math(test_dataset, tokenizer)
    ...
    output_file = "Mistral_Inference_Competition_Dataset.csv"
...
if __name__ == "__main__":
   mistral_competition_inference_with_resume()