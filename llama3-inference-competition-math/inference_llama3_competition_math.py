...
def competition_math_inference():
    ...
    model, tokenizer = FastLanguageModel.from_pretrained(
        model_name = "knazarali3/llama3_COMPETITION_MATH", # YOUR MODEL YOU USED FOR TRAINING
    ...
    for start_idx in tqdm(range(0, len(dataset["text"]), batch_size), desc="Processing Batches"):
        ...
        batch_ids = dataset["row_index"][start_idx:start_idx + batch_size]  # Assuming 'id' is the column name for question IDs
    ...
    results_df.to_csv("inference_llama3_competition_math.csv", index=False)
